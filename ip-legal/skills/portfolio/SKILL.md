...
6. **`--audit`:** Mode 5. Broader health check — deadline hygiene,
   registration gaps, use/non-use exposure (§8-approaching marks for
   US-designated assets; *caducidade* risk under LPI art. 143 for
   Brazilian marks idle 5+ years), owner inconsistencies, expiration
   horizon, unwatched marks.
...
8. **Guardrail reminder:** Computed deadlines are reference only. Every
   output closes with a line directing verification against the e-INPI /
   Sistema INPI, WIPO Madrid Monitor, or relevant registry before filing
   or paying. A
...
- **e-INPI directly (Sistema INPI, via login gov.br / procurador cadastrado)**
  — pulls status, deadlines, and correspondence for your whole portfolio
  rather than one application at a time. Not currently available as an MCP;
  on the wish list in `CONNECTORS.md`.
...
> deadlines against the e-INPI / Sistema INPI, WIPO Madrid Monitor /
> Patentscope, EUIPO eSearch, USPTO TSDR (for US-designated assets), UKIPO
> online records, or the relevant national registry before acting.** If you use
> Anaqua, CPA Global, Clarivate, Alt Legal, or another IP management system,
> their docket is authoritative for your assets — use this tracker to organize
> and surface their data, not to replace it.
>
...
- **Brazilian patents (INPI, LPI arts. 40, 84-86):** Annual fees (*retribuição
  ...
  Term is 20 years from filing for patentes de invenção, 15 years from filing
  for modelos de utilidade. LPI art. 40's sole paragraph originally guaranteed
...
- **Madrid International trademarks:** 10-year registration term renewable at
  ...
  requirements on top (e.g., Brazil's *caducidade* exposure above, or the US
  §71 declaration for US designations).
- **US trademarks (for US-designated or US-registered assets):** §8
  Declaration of Use between 5th and 6th anniversary of registration (or §71
  for Madrid designations), then combined §8/§9 renewal at 10 years and every
  10 years thereafter. §15 Incontestability available after 5 years of
  continuous use. 6-month grace period with surcharge for §8 and §9; no grace
  for the underlying use itself.
- **EUIPO trademarks:** 10-year renewal; 6-month grace with surcharge.
- **US utility patents (for US-designated or US-granted assets):** Maintenance
  fees due at 3.5, 7.5, and 11.5 years from grant. 6-month grace window with
  surcharge; after that, potential revival by petition if lapse was
  unintentional.
...
- **Copyright (Brazil, Lei 9.610/1998 art. 18):** No registration or renewal
  is required for protection — copyright arises automatically on creation.
  Registration is optional and non-constitutive: literary/artistic works at
  the Biblioteca Nacional, software at INPI (Lei 9.609/1998, sui generis
  regime). This is a fundamental difference from jurisdictions with a
  registration-dependent system — flag it if a portfolio migrated from one
  assumes a maintenance obligation that doesn't exist here.
...
# Generated: [date]
# Last updated: [date]
# Disclaimer: computed deadlines are reference only — confirm with e-INPI/WIPO/
...
assets:
  - id: "TM-BR-001"
    ...
    application_number: "[processo INPI or null]"
    registration_number: "[number or null]"
    classes: ["9", "42"]                       # Nice classes (NCL) for TM; CPC/IPC for patents; null otherwise
    ...
    next_deadlines:                            # computed; refreshed on --report and --audit
      - type: "Renewal (LPI art. 133/143)"
        due_date: "[YYYY-MM-DD]"
        grace_end: "[YYYY-MM-DD or null]"
        basis: "10-year term from grant; renewal window in the final year"
        action: "File renewal request with INPI"
        status: "upcoming"                     # upcoming / due_soon / overdue / grace / filed
    use_in_commerce: true                      # TM only — no INPI mid-term use affidavit, but 5+ years of
                                                # non-use exposes the mark to third-party caducidade (LPI art. 143)
  ...
  - id: "PAT-US-001"
    ...
    next_deadlines:
      - type: "3.5-year maintenance fee"
        ...
        status: "upcoming"
    claims_count: 20
    entity_size: "large"                       # large / ME-EPP (Lei Complementar 123/2006) / individual / startup
                                                # — drives INPI fee category; other offices (USPTO large/small/
                                                # micro) use a different schedule for that jurisdiction's assets
    docket_id: null
...
❓ UNKNOWN ([N])
  ...
  Confirm with [IP management system / e-INPI / relevant registry] before relying on this report.
...
Close the report with the caveat line: *"Computed from portfolio register. Verify each deadline against the e-INPI/WIPO/registry of record before filing or paying."*
...
> (or with the e-INPI / WIPO Madrid Monitor / relevant registry)? If yes,
> proceed. If no:
>
...
>
> If you need to find a licensed attorney or Agente da Propriedade Industrial in your jurisdiction: your professional regulator's referral service
> is the fastest starting point (OAB — seccional local — in Brazil, state bar in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent).
...
**Use / non-use exposure (TM only)**
- For US-designated assets: §8 approaching on a mark flagged
  `use_in_commerce: false` or uncertain? The §8 requires use; mark needs a
  use audit before filing or an excusable nonuse declaration.
- For Brazilian assets: any mark flagged `use_in_commerce: false` or
  uncertain, and 5+ years since registration with no evidence of use?
  Exposed to a third-party *caducidade* action (LPI art. 143) — flag for a
  use audit or a defensive use program before a challenge lands.
...
USE / NON-USE EXPOSURE (TM)
  §8 approaching on uncertain-use marks (US-designated): [list]
  Caducidade risk — 5+ years uncertain use (Brazilian marks): [list]
...
- It does not verify deadlines against the e-INPI, WIPO, or any other
...
- It does not read office records to verify status. A renewal or annuity
  shown as "filed" here means someone told it so — not that INPI (or the
  USPTO, for a US-designated asset) accepted it. Confirm acceptance through
  e-INPI/TSDR or the IP management system.

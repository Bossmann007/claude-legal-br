...
**QA-relevant jurisdiction check for installed skills.** Community skills — a maioria escrita para o mercado americano — comumente afirmam um header de "attorney work product" (`PRIVILEGED & CONFIDENTIAL — ATTORNEY WORK PRODUCT — PREPARED AT THE DIRECTION OF COUNSEL`). "Attorney work product" é doutrina americana (FRCP 26(b)(3)) e não existe como tal no direito brasileiro — afirmar isso num documento não cria a proteção. No Brasil, o que protege análise jurídica interna e comunicação com o cliente é o **sigilo profissional do advogado** (Estatuto da OAB, Lei 8.906/1994, art. 34, VII, e art. 7º, II — inviolabilidade do escritório e da correspondência profissional), regime distinto do work-product privilege americano (não cobre, por exemplo, documento preparado por não-advogado sem relação de assessoria jurídica). Ao fazer QA de uma skill instalada, sinalize qualquer header que afirme proteção de work-product americana sem nota condicional de jurisdição — uma falsa garantia de proteção é pior que nenhuma marcação. Recomende à skill adicionar um branch de jurisdição mantendo `PRIVILEGED & CONFIDENTIAL` (com sentido em qualquer sistema) e substituindo por `CONFIDENCIAL — SIGILO PROFISSIONAL (ART. 34, VII, LEI 8.906/1994) — ANÁLISE INTERNA, NÃO SUBSTITUI PARECER DE ADVOGADO EXTERNO` quando o perfil de prática for brasileiro.

**Non-lawyer output mode.** Quando o perfil de prática indica que o usuário não é advogado, as próprias saídas voltadas ao usuário do hub — o relatório do `related-skills-surfacer`, os resultados do `registry-browser`, o veredito do `skills-qa`, e as confirmações de instalação/atualização — se estruturam para um leitor que não decodifica jargão jurídico: (1) o resumo para o advogado (o que um advogado supervisor precisa saber sobre a instalação, atualização ou skill proposta) vai no topo, não enterrado, (2) toda sinalização jurídica ganha uma glosa em português simples entre parênteses, (3) toda citação legal ganha um assunto em português simples. Exemplo: "Sinalização: possível questão de aviso prévio em dispensa coletiva (CLT, art. 477 c/c entendimento do TST sobre negociação coletiva prévia) — dispensa em massa sem negociação prévia com o sindicato pode ser considerada abusiva." Teste: o leitor conseguiria levar a saída ao advogado supervisor e explicar sem um advogado na sala? O hub também repassa o sinal de Role para as skills instaladas — se a seção `## Outputs` de uma skill tem modo não-advogado, o hub garante que Role seja legível onde a skill espera.
...
Community skills instaladas via este hub têm, em geral, frameworks, testes, estatutos e procedimentos americanos como padrão (é o mercado de origem do lpm-skills e do awesome-legal-skills). Quando o usuário, a matéria, ou os fatos envolvem o direito brasileiro — ou qualquer jurisdição diferente da assumida pela skill instalada — reconheça isso e aja, não aplique silenciosamente doutrina americana a fatos brasileiros.

1. **Detecte.** Confira o footprint de jurisdição no perfil de prática (`~/.claude/plugins/config/claude-for-legal/legal-builder-hub/CLAUDE.md` e `company-profile.md`). Confira os fatos da matéria (lei aplicável, localização das partes, onde o produto é vendido, onde estão as pessoas afetadas). Se qualquer um disso for Brasil (ou outra jurisdição não-americana), o framework padrão da skill instalada pode não se aplicar.
2. **Avalie.** A skill instalada tem framework para esta jurisdição? Os plugins de primeira parte deste marketplace (ai-governance-legal, employment-legal, ip-legal etc.) já carregam substância BR real — LGPD, CLT, Lei 9.279/1996 etc. Uma community skill instalada via este hub provavelmente não. Se tiver, use.
3. **Se não tiver framework:** diga isso claramente: "Esta análise usa um framework americano ([o teste/estatuto]). Você está no Brasil, onde o direito é diferente — CLT em vez de employment-at-will, LGPD em vez de CCPA, CDC em vez de consumer-protection state law, etc. Aplicar doutrina americana aqui daria uma resposta errada que parece certa."
4. **Ofereça o próximo passo na árvore de decisão:**
   - **Buscar o padrão aplicável.** Se houver conector de pesquisa disponível (JusBrasil, STF, STJ, Planalto), busque "[tema] direito brasileiro" e reporte o que encontrar, marcado `[verificar contra fonte primária]`.
   - **Encaminhar para especialista.** "Um advogado brasileiro da área deveria decidir isso. Pergunte a ele: [a pergunta específica]."
   - **Sinalizar a lacuna e continuar com ressalva.** "Vou rodar o framework americano como estrutura inicial, mas toda conclusão fica marcada `[framework americano — verificar contra direito brasileiro]`."
5. **Nunca produza uma resposta confiante usando o direito da jurisdição errada.** Confiante-e-errado é pior que incerto-e-sinalizado. Um advogado que pega você aplicando *employment-at-will* a uma rescisão trabalhista brasileira (regida pela CLT, com verbas rescisórias, aviso prévio, FGTS) para de confiar no resto.
...
- `[JusBrasil]` / `[Escavador]` / `[PJe]` / `[STF]` / `[STJ]` / `[Planalto]` / `[user provided]` — where a cite actually came from. Provenance, not confidence. Only use these when the cite literally appeared in that source in this session.
- **`[settled — last confirmed YYYY-MM-DD]`** — stable statutory and regulatory references that have been checked against a primary source on the stated date. The date matters: "stable" references change. A alteração da LGPD pela Lei 13.853/2019 (criação da ANPD) ou uma resolução nova do CNJ sobre PJe mudam o que era `[settled]` antes da vigência. O prazo de vigência do Marco Legal das Startups, ou uma súmula vinculante nova do STF, também deslocam a data. The date tells the reader when the confidence was earned and whether it's earned it lately. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead — an unconfirmed "settled" is the confident overclaim we built the whole attribution system to prevent.

A reviewer-note shorthand like "JusBrasil verified" is honest only when a research tool actually returned the cite — it describes what the tool did, not what the skill's output is. A skill's output is never "verified" by the skill itself; the reader is what verifies. The QA check (`/legal-builder-hub:skills-qa`) looks for this discipline in community skills; skills that claim their own output is verified fail the trust-surface check.

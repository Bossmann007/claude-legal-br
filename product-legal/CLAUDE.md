...
- **Brasil (default):** The **sigilo profissional do advogado** / prerrogativa da defesa under the Estatuto da OAB (Lei 8.906/1994), art. 7º, II e XIX, protects attorney-client communications and the advogado's own case analysis from disclosure and search. It is narrower than US work product: a CADE dawn raid, or a court order under CPC arts. 396-404 (exibição de documento), can still compel production of certain documents, subject to the refusal grounds in art. 404 (e.g., matters covered by professional secrecy). This protection applies only when the document was effectively produced or supervised by a lawyer with active OAB registration.
- **US jurisdictions (exception):** Where the matter footprint touches the US, "Attorney work product" is a discovery-shielding doctrine (FRCP 26(b)(3)) that operates differently from Brazilian sigilo. For US-touching matters, consider: `PRIVILEGED & CONFIDENTIAL — ATTORNEY WORK PRODUCT — PREPARED AT THE DIRECTION OF COUNSEL`, with a note: `[Note: work product protection is a US discovery doctrine. In Brazil, the operative regime is sigilo profissional do advogado (Estatuto OAB, Lei 8.906/1994). Confirm which regime applies to your matter's jurisdiction footprint.]` Asserting "attorney work product" on a Brazilian document invokes a US concept a Brazilian court or regulator has no obligation to recognize.
...
> "You mentioned a 5-year prescrição for this labor claim regardless of employment status — my understanding is it's 5 years during the employment relationship, capped at 2 years after termination (CF art. 7º, XXIX). Can you confirm which you meant? `[premise flagged — verify]`"
...
- **`[settled — last confirmed YYYY-MM-DD]`** — stable statutory and regulatory references that have been checked against a primary source on the stated date. The date matters: "stable" references change. The LGPD's (Lei 13.709/2018) ANPD regulations and the Marco Civil da Internet (Lei 12.965/2014) secondary rules have been amended repeatedly since 2020, and PL 2338/2023 (the AI bill) has moved between chambers and versions — a reference that was `[settled]` last year may not be today. The date tells the reader when the confidence was earned and whether it's earned it lately. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead — an unconfirmed "settled" is the confident overclaim we built the whole attribution system to prevent. Note: never use `[settled]` for a platform policy — those change without notice.

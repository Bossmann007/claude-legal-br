...
**Connect a research tool first — the citation guardrails depend on it.** Without one, every cite is tagged `[verify]` and the reviewer note above each deliverable records that sources weren't verified. Skills work either way; a research tool (JusBrasil, Escavador, or PJe) just shifts verification work off your plate.
...
Some features reference external integrations (document management, launch trackers, legal research (JusBrasil, Escavador, PJe), regulatory feeds). Brazil has no formal eDiscovery-platform regime — document production runs through CPC arts. 396-404 (exibição de documento), not a discovery-platform tool, so no such tool is assumed here. These integrations are not bundled — if you have an MCP server for one of these in your environment, the relevant features will use it. Without one, the plugin falls back to file upload and manual workflows. Run `/product-legal:cold-start-interview --check-integrations` to see what's available in your environment.

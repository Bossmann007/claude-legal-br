...
- If Role is **Lawyer / legal professional**: `CONFIDENCIAL — SIGILO PROFISSIONAL (ADVOGADO-CLIENTE) — PREPARADO SOB ORIENTAÇÃO DA ASSESSORIA JURÍDICA`
- If Role is **Non-lawyer** (either type): `NOTAS DE PESQUISA — NÃO CONSTITUI PARECER JURÍDICO — REVISAR COM ADVOGADO INSCRITO NA OAB ANTES DE AGIR`

**A proteção do cabeçalho depende de fundamento legal real, não da etiqueta.** O Brasil não tem o instituto americano de "attorney work product" (FRCP 26(b)(3)). O que existe é o **sigilo profissional do advogado**, previsto no Estatuto da OAB (Lei 8.906/1994, art. 7º, II e XIX, e art. 34, VII) e no Código de Ética e Disciplina da OAB — protege comunicações e documentos produzidos no exercício da advocacia, mas:

- **Não protege automaticamente qualquer documento interno de RH.** Um relatório de auditoria trabalhista ou uma análise de compliance preparada pelo departamento pessoal (sem envolvimento de advogado) não tem, por si só, sigilo profissional — pode ser exigido em fiscalização do Ministério do Trabalho e Emprego (MTE) ou em processo na Justiça do Trabalho.
- **Fiscalização trabalhista tem poderes amplos.** Auditores fiscais do trabalho (Lei 10.593/2002) podem requisitar documentos, notificar e autuar diretamente — o sigilo profissional não blinda a empresa de fiscalização, apenas protege a comunicação especificamente com o advogado.
- **Investigações internas** (assédio, denúncias) que envolvem advogado desde o início, com objetivo de orientação jurídica, têm proteção mais forte do que uma investigação conduzida só por RH.

**Se o rodapé da empresa tem operação fora do Brasil,** ajuste o cabeçalho:
- Mantenha `CONFIDENCIAL` (a marcação de confidencialidade tem valor em qualquer jurisdição).
- Acrescente nota: `[Nota: a proteção aqui é o sigilo profissional advogado-cliente (Lei 8.906/1994), não o "work product" americano. Em [jurisdição], o regime de confidencialidade pode ser diferente — confirmar antes de usar esta marcação para blindar o documento de divulgação.]`

Uma falsa garantia de proteção é pior que nenhuma marcação. O advogado que confia em "sigilo profissional" para blindar um relatório de investigação que na verdade foi conduzido só pelo RH, sem advogado, é quem perde o argumento na Justiça do Trabalho.

*Remova o cabeçalho de entregáveis voltados para fora (cartas de oferta enviadas a candidatos, cartas de dispensa, termos de rescisão/TRCT circulados com o empregado, respostas a órgãos fiscalizadores) — ver instruções do skill específico. O sigilo depende de fatos além da etiqueta; o skill internal-investigation tem requisitos adicionais de formação do sigilo.*

**Modo de output para não-advogado.** Quando o perfil da prática indica que o usuário não é advogado, estruture o output para um leitor que não decodifica jargão jurídico: (1) o resumo para o advogado vai no topo, não escondido, (2) todo flag jurídico ganha um glosa em português simples entre parênteses, (3) toda citação legal ganha um título em linguagem simples. Exemplo: "Flag: possível estabilidade gestante não observada (CF art. 10, ADCT, e Súmula 244 TST) — gestante tem estabilidade desde a confirmação da gravidez até 5 meses após o parto." Teste: o leitor conseguiria levar o output ao chefe e explicar sem um advogado na sala?
...
If everything is green (research tool connected, full read, no flags, currency checked), collapse to one line: `⚠️ Reviewer note: JusBrasil/Escavador verified · full read · no flags · ready for your eyes`. Don't pad with bullets that all say "no issues."
...
- **`[settled — last confirmed YYYY-MM-DD]`** — stable statutory and regulatory references that have been checked against a primary source on the stated date. The date matters: "stable" references change. A resolução da ANPD sobre transferência internacional de dados mudou os requisitos em 2024, o que teria sido `[settled]` antes disso. O marco legal de IA já teve o prazo de vigência adiado mais de uma vez. The date tells the reader when the confidence was earned and whether it's earned it lately. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead — an unconfirmed "settled" is the confident overclaim we built the whole attribution system to prevent.
...
- `[JusBrasil]` / `[Escavador]` / `[PJe]` / `[statute / regulator site]` / `[user provided]` — where a cite actually came from. Provenance, not confidence. Only use these when the cite literally appeared in that source in this session.
...
A reviewer-note shorthand like "JusBrasil/Escavador verified" is honest only when a research tool actually returned the cite — it describes what the tool did, not what the skill's output is. The skill's output is never "verified" by the skill itself; the reader is what verifies.
...
> "You mentioned um prazo prescricional de 5 anos para verbas trabalhistas — meu entendimento é que a prescrição é de 2 anos após a extinção do contrato, limitada a 5 anos retroativos (art. 7º, XXIX, CF). Pode confirmar o que você quis dizer? `[premise flagged — verify]`"
...
**Estados/municípios com empregados (CLT):** [PLACEHOLDER — lista]
**Países com empregados (expatriados/expansão):** [PLACEHOLDER — lista]
**Regime:** [PLACEHOLDER — remoto/home office (CLT art. 75-A a 75-E) / presencial / híbrido]

**Bases sindicais e Convenções Coletivas de Trabalho (CCT) relevantes** (mais empregados, cláusulas mais restritivas, ou histórico de reclamações trabalhistas):
- [PLACEHOLDER — ex.: sindicato da categoria em SP, CCT vigente, data-base]

---

## Contratação (hiring review)

**Quando o jurídico revisa contratações:** [PLACEHOLDER — todas as ofertas / só cargos de confiança/executivos / só com cláusula de não-concorrência]

**Modelo de carta-proposta / contrato de trabalho:** [PLACEHOLDER — local]
**Política de não-concorrência e não-aliciamento pós-contratual:** [PLACEHOLDER — cláusula de não-concorrência exige indenização compensatória enquanto durar a restrição (Súmula 6 TST / entendimento consolidado); confirmar contrapartida financeira prevista]
**Política de verificação de antecedentes:** [PLACEHOLDER — atenção à LGPD: dado de antecedente criminal é dado sensível/pessoal, exige base legal e finalidade específica]
**eSocial / admissão:** [PLACEHOLDER — responsável pelo registro no eSocial dentro do prazo legal]

---

## Rescisão (termination review)

**Quando o jurídico revisa rescisões:** [PLACEHOLDER — todas / só sem justa causa em massa / só cargos de confiança/executivos / dispensas com indício de estabilidade]

**Verbas rescisórias padrão:** [PLACEHOLDER — aviso prévio (proporcional, Lei 12.506/2011), 13º proporcional, férias + 1/3, saldo de salário, multa FGTS 40% (ou 20% em acordo/distrato)]
**Homologação / TRCT:** [PLACEHOLDER — quitação no sistema eSocial/Portal Emprega Brasil; assistência sindical recomendada em casos sensíveis, art. 477 CLT]
**Prazo de pagamento das verbas rescisórias:** [PLACEHOLDER — 10 dias corridos a contar do término do contrato, art. 477 §6º CLT — atraso gera multa de 1 salário do empregado]

**Flags de alto risco em rescisão (escalar automaticamente):**
- [PLACEHOLDER — ex.: estabilidade gestante (CF art. 10 ADCT), estabilidade de membro da CIPA (art. 10, II, "a" ADCT), estabilidade acidentária (Lei 8.213/1991 art. 118 — 12 meses após alta do INSS), dirigente sindical (art. 8º, VIII CF), empregado com CAT aberta, denúncia de assédio recente, próximo à aposentadoria]

---

## Regulamento interno / política de RH

**Versão atual:** [PLACEHOLDER — local, data]
**Periodicidade de atualização:** [PLACEHOLDER]
**Adendos regionais (CCTs por base territorial):** [PLACEHOLDER — quais bases têm cláusulas específicas de CCT]

---

## Jornada e remuneração (wage & hour)

**Revisão de enquadramento (cargo de confiança/gestão vs. controlado, CLT art. 62):** [PLACEHOLDER — quando, por quem]
**Revisão de classificação PJ vs. CLT (risco de pejotização/vínculo):** [PLACEHOLDER]
**Política de horas extras e banco de horas:** [PLACEHOLDER — adicional mínimo 50% (CF art. 7º, XVI), acordo de compensação/banco de horas conforme CLT art. 59]
**Áreas de risco de classificação conhecidas:** [PLACEHOLDER — cargos em fronteira entre CLT art. 62 (sem controle de jornada) e jornada controlada]

---

## Regras de escalonamento por base territorial/CCT

*Construído a partir do regulamento interno + memorandos de rescisão no cold-start.*

| Base/CCT | Regras especiais | Escalar quando |
|---|---|---|
| [PLACEHOLDER — ex.: CCT do sindicato de SP] | [Assistência sindical obrigatória na rescisão, piso salarial diferenciado etc.] | [Qualquer rescisão, qualquer cláusula de não-concorrência] |

---

## Sistemas

**HRIS/folha:** [Sistema / nenhum]
**Acesso a dados de licenças/afastamentos:** [Jurídico tem acesso de leitura / manual — `~/.claude/plugins/config/claude-for-legal/employment-legal/leave-register.yaml`]
**eSocial / responsável pelo envio:** [PLACEHOLDER]
**Local do regulamento interno:** [Drive / SharePoint / caminho local]

---

## Escalonamento

| Questão | Trata em | Escala para | Quando |
|---|---|---|---|
| Carta-proposta de rotina | [RH] | [Você] | Não-concorrência, cargo executivo, nova base territorial |
| Rescisão por desempenho | [RH + você] | [Diretoria jurídica] | Flags de alto risco presentes |
| Dispensa em massa / PDV | — | [Diretoria jurídica + escritório externo] | Sempre (Lei 13.467/2017 e Convenção 158 OIT — negociação coletiva prévia recomendada) |
| Fiscalização (MTE, Justiça do Trabalho, sindicato) | — | [Diretoria jurídica imediatamente] | Sempre |

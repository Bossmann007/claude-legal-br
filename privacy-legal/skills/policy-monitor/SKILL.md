...
- `## Who we are` → `## Regulatory footprint` — the regimes in scope (LGPD as the baseline national framework — no US-style state-by-state patchwork to track — plus GDPR if EU data subjects are involved, and sectoral overlays such as Res. BCB/CMN + LC 105/2001 for financial data, LGPD art. 11 + Resoluções ANVISA/ANS/CFM for health data, LGPD art. 14 + ECA arts. 17-18 for children/adolescents, etc.)
...
4. **Sector-specific notices.** Financial-data notices under Res. BCB/CMN 4.658/2018 + LC 105/2001, health-data notices under LGPD art. 11 + Resoluções ANVISA/ANS/CFM, and children/adolescent notices under LGPD art. 14 + ECA (Lei 8.069/1990) arts. 17-18. These have their own update obligations and their own consistency requirements with the general privacy policy. (Detail below under "Sectoral notices.")
...
A company with a clean privacy policy and a stale App Store label is a company with an ANPD enforcement action waiting to happen. Sweep the surfaces, not just the document.

### Sectoral notices are in scope for this sweep

The website privacy policy is one notice. Sectorally-regulated practices require a separate, sector-specific notice that the website policy does not substitute for. If `## Regulatory footprint` includes any of the following, the sweep diffs practice against that notice in addition to the website policy — or flags its absence if no such notice has been configured:

| Footprint entry | Sectoral notice to diff against | What to flag |
|---|---|---|
| **Financial data** (Res. BCB/CMN, e.g. Res. CMN 4.658/2018 + LC 105/2001 sigilo bancário) | Financial-data privacy notice tied to the institution's regulatory disclosures | Outputs implying new categories of financial/account data, sharing with non-affiliated third parties, or changes to opt-out mechanics that the notice doesn't reflect. A DPA signed with an analytics vendor receiving financial data with no matching notice update is a gap. |
| **LGPD art. 11 — dado sensível de saúde** (Resoluções ANVISA/ANS/CFM as applicable) | Health-data processing notice (Brazil has no direct BAA/NPP equivalent — flag as a thinner regime) | Outputs implying new uses or disclosures of health data, new categories, or changes to patient-rights mechanics. A processing agreement signed with a new subcontractor flowing sensitive health data with no matching notice refresh is a gap. |
| **Education records** | No dedicated Brazilian statute equivalent to a student-records-specific notice regime — general LGPD controller/operator obligations apply `[verify]` | Outputs implying new disclosure categories to education service providers, new data elements collected from students, or changes that implicate guardian-consent flow-through for minors (see LGPD art. 14 below). |
| **LGPD art. 14 + ECA arts. 17-18** (children and adolescents) | Direct notice to parents/guardians + age-appropriate notice — flag Brazil's gap vs. COPPA's actual-knowledge / verifiable-parental-consent machinery, which LGPD does not spell out with the same specificity | Outputs implying new data categories collected from children/adolescents, new third-party disclosures, or changes to the parental/guardian-consent mechanic. |
| **Other sectoral regime** | The regime's specific notice or consent requirement | Processing activities the regime restricts that aren't reflected in the configured notice. |
...
**If the sweep cannot locate the sectoral notice**, say so explicitly — do not silently default to diffing only against the website policy. A fintech DPO relying on a policy-monitor sweep that ignored the financial-data regime would ship with an outdated regulator-facing notice and no warning. Surface the gap loudly.

**Ask the user if the footprint is ambiguous.** If `## Regulatory footprint` says "LGPD / GDPR" but the outputs scan surfaces health data, financial data, or student data categories, surface the footprint-vs-practice mismatch before proceeding: "Your footprint doesn't list [the financial-data regime / LGPD art. 11 health-data regime / LGPD art. 14 + ECA] but this sweep is looking at outputs that involve [category]. Should this regime be added to the footprint, and is there a sectoral notice to diff against?"

...
[Company] is a [B2B SaaS / consumer app / etc.]. We are primarily um [controlador / operador / ambos]
(Art. 5º, VI e VII, LGPD) com relação a [de quem são os dados]. Dados residem em [regiões]. Equipe de privacidade tem [N] pessoas.
**Encarregado (DPO — Art. 41 LGPD):** [nome ou "não indicado — obrigatório para controlador, dispensa possível para agente de pequeno porte conforme Res. CD/ANPD nº 2/2022"]. Escalação vai para [nome].
...
| Notificação de incidente ao controlador | [PLACEHOLDER — em "prazo razoável", Art. 48 LGPD; recomendar contratualmente ≤48h para permitir ao controlador cumprir seu próprio dever de comunicação à ANPD] | | |
| Troca de suboperador | [PLACEHOLDER] | | |
| Localização dos dados | [PLACEHOLDER — atenção a transferência internacional, Art. 33 LGPD: exige cláusulas-padrão contratuais, selo/certificado ANPD, ou uma das hipóteses do Art. 33] | | |
| Eliminação ao término do contrato | [PLACEHOLDER — dever de eliminação, Art. 15/16 LGPD, ressalvadas hipóteses de guarda obrigatória] | | |
| Responsabilidade solidária | [PLACEHOLDER — Art. 42, §1º: operador responde solidariamente se descumprir LGPD ou instruções lícitas do controlador] | | |

### Quando somos controlador

| Termo | Exigimos | Aceitável | Nunca aceitar |
|---|---|---|---|
| [PLACEHOLDER] | | | |
| Sub-rogação de responsabilidade pelo operador | Cláusula de indenização por descumprimento do operador (Art. 42, §1º) | | Cláusula que isente operador de qualquer responsabilidade por vazamento causado por ele |

### O ponto inegociável

[PLACEHOLDER — cláusula que quebra o negócio da DPA]

### Sanções administrativas (Art. 52 LGPD) — referência de dosimetria

Advertência → multa simples até 2% do faturamento (limitada a R$ 50 milhões por infração) → multa diária → publicização da infração → bloqueio/eliminação dos dados → suspensão parcial/total do banco de dados → proibição de atividade de tratamento. A ANPD aplica dosimetria conforme Res. CD/ANPD nº 4/2023 (gravidade, boa-fé, reincidência, vantagem auferida, cooperação). Usar essa escala ao avaliar exposição de risco em qualquer não conformidade encontrada.
...
## Atendimento a requisições de titular (Art. 18 LGPD)

**Volume:** [PLACEHOLDER]
**Responsável:** [PLACEHOLDER]
**Sistemas a verificar:** [PLACEHOLDER — listar onde dados do titular residem]
**Verificação de identidade:** [PLACEHOLDER]
**Prazo de resposta:** [PLACEHOLDER — LGPD não fixa prazo geral único; Res. CD/ANPD nº 15/2024 e boas práticas de mercado usam 15 dias para confirmação de existência de tratamento (Art. 19, §1º analógico ao regime bancário) e "prazo razoável" para os demais direitos do Art. 18 — definir SLA interno de referência, recomendado 15 dias corridos, prorrogável uma vez por igual período mediante justificativa ao titular]
**Direitos do titular cobertos (Art. 18):** confirmação de tratamento · acesso · correção · anonimização/bloqueio/eliminação de dados desnecessários ou excessivos · portabilidade · eliminação de dados tratados com consentimento · informação sobre compartilhamento · informação sobre não fornecer consentimento e consequências · revogação do consentimento
...
- **Brasil:** Não existe doutrina de "work product" no direito brasileiro. A proteção equivalente é o **sigilo profissional do advogado** (Art. 7º, XIX, Estatuto da OAB — Lei 8.906/1994; Art. 34-36 Código de Ética e Disciplina da OAB), que protege comunicações e documentos produzidos no exercício da advocacia contra busca e apreensão sem acompanhamento de representante da OAB (Art. 7º, §6º-B). Análises internas de compliance, DPIAs e pareceres preparados por equipe interna sem advogado responsável NÃO gozam da mesma proteção — a presença de advogado inscrito na OAB como autor ou supervisor do documento é o que ativa o sigilo. Rotular um documento como "sigiloso" sem envolvimento de advogado não cria a proteção.
...
- `[statute / regulator site]` — ONLY if you fetched the text from the regulator's website (gov.br/anpd or an official source) in this session.
...
- **`[settled — last confirmed YYYY-MM-DD]`** — stable statutory and regulatory references that have been checked against a primary source on the stated date. The date matters: "stable" references change. ANPD's own regulamentações (Res. CD/ANPD) have been revised repeatedly — a reference `[settled]` last year may not be today. The date tells the reader when the confidence was earned and whether it's earned it lately. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead — an unconfirmed "settled" is the confident overclaim we built the whole attribution system to prevent.

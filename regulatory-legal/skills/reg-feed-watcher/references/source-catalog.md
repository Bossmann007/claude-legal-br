...
DOU has no single structured API like Federal Register — RSS/scraping-based, split
across três seções. Agency portals are individually maintained on gov.br with
uneven feed support; several require email/page-watch, same pattern as the US
state table below.

| Source | Feed URL | Format | Covers | Auth | Notes |
|---|---|---|---|---|---|
| DOU — Seção I | `https://www.in.gov.br/leiturajornal?secao=do1` (site); `https://www.in.gov.br/consulta/-/buscar/dou` (search) | HTML + JSON search endpoint (unofficial) | Leis, decretos, atos normativos gerais | None | No official public API `[verify]`. Third-party scrapers exist (e.g. querido-diario). Seção I is the one that matters for "new obligation" alerts. |
| DOU — Seção II | `https://www.in.gov.br/leiturajornal?secao=do2` | HTML | Atos de pessoal (nomeações, portarias individuais) | None | Lower relevance for regulatory monitoring; usually skip unless tracking specific appointments. |
| DOU — Seção III | `https://www.in.gov.br/leiturajornal?secao=do3` | HTML | Contratos, licitações, avisos | None | Relevant for Lei 14.133/2021 (licitações) monitoring. |
| ANPD | `https://www.gov.br/anpd/pt-br/assuntos/noticias` | HTML | LGPD regulamentações, RIPD guidance, sanções, consultas públicas | None | ⚠️ No public RSS located. ANPD also publishes normative acts via DOU Seção I — cross-check both. |
| BCB / CMN | `https://www.bcb.gov.br/estabilidadefinanceira/normativos` | HTML + some RSS (`bcb.gov.br/rss`) | Resoluções BCB/CMN (Res. 4.658/2018 etc.), circulares, comunicados | None | `bcb.gov.br/rss` hub has multiple topic feeds — verify per Selic/normativos/comunicados. |
| ANVISA | `https://www.gov.br/anvisa/pt-br/assuntos/noticias` | HTML + RSS option | Resoluções (RDC), consultas públicas, alertas sanitários | None | RSS feeds advertised per-section on gov.br; verify current URL. |
| ANATEL | `https://www.gov.br/anatel/pt-br/assuntos/noticias` | HTML | Resoluções, atos, consultas públicas (telecom) | None | ⚠️ No consolidated public RSS located. |
| ANEEL | `https://www.gov.br/aneel/pt-br/assuntos/noticias` | HTML | Resoluções normativas, consultas públicas (energia) | None | ⚠️ No consolidated public RSS located. |
| ANS | `https://www.gov.br/ans/pt-br/assuntos/noticias` | HTML | Resoluções normativas (saúde suplementar) | None | ⚠️ No consolidated public RSS located. |
| CADE | `https://www.gov.br/cade/pt-br/assuntos/noticias` | HTML + RSS (`cade.gov.br` legacy feed, verify) | Atos de concentração, processos administrativos, guias | None | Also publishes via DOU Seção I for normative acts. |

**Consulta pública (public comment) note:** unlike US notice-and-comment under the APA (fixed comment windows, uniform docket structure via Regulations.gov), Brazilian consulta pública is agency-specific — no uniform clock, no single tracking portal. Each agency (ANPD, BCB, ANATEL, etc.) runs its own consultation process and timeline `[verify per agency]`.
...
**Regulatory in-house team (Brazil):**
DOU Seção I (page watch or scraper), ANPD, BCB/CMN (if regulated entity), sector
agency as applicable (ANVISA / ANATEL / ANEEL / ANS / CADE), JusBrasil/Escavador/PJe
for judicial developments. Add IAPP + Lexology for aggregator coverage on
cross-border privacy/AI topics.

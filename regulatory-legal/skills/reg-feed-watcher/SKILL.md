...
- **Diário Oficial da União (DOU)** (`in.gov.br`)
  — the federal official gazette, published daily across seções I (atos normativos),
  II (atos de pessoal), III (contratos/licitações). Unlike the US Federal Register,
  there's no single structured JSON API returning document type, effective date, and
  comment/consultation deadline in one call — coverage here is via the in.gov.br
  search portal or RSS where the section publishes one, and typically needs
  post-processing to extract effective date and any consulta pública deadline.
- **Agência gov.br portal** — each Brazilian regulator publishes its own normativos
  and consultas públicas on its own gov.br subdomain (gov.br/anpd, gov.br/bcb,
  gov.br/anvisa, gov.br/anatel, gov.br/aneel, gov.br/ans, gov.br/cade) — treat these
  as the structured-detail source for sector rules, with DOU as the catch-all.
- **Direct regulator RSS** — fetch and parse any RSS URLs in ~/.claude/plugins/config/claude-for-legal/regulatory-legal/CLAUDE.md feed
  configuration (ANPD, BCB/CMN, ANVISA, ANATEL, ANEEL, ANS, CADE, non-Brazilian regulators, etc.).

Reference for common watchlist regulators (a portal, not a structured API slug — Brazilian agencies don't expose the Federal Register's agency-slug pattern):
| Regulator | Portal |
|---|---|
| ANPD | gov.br/anpd |
| BCB / CMN | bcb.gov.br |
...
For any regulator not in this list: check the agency's own gov.br page for a
"notícias" or "publicações" RSS, or fall back to DOU search + manual entry.
...
- **JusBrasil / Escavador / PJe MCP:** Same, for judicial developments (enforcement litigation, appeals of agency decisions) tied to the watchlist.
...
|---|---|
| Norma final (resolução, instrução normativa, portaria em vigor) | Usually "always material" |
| Proposta em consulta pública | Usually "review-worthy" — and always log the consultation deadline |
| Tomada pública de subsídios / audiência pública pré-normativa `[terminology varies by agency — model knowledge, verify]` | Review-worthy for **strategy**, not compliance — no imposed requirements yet, but signals direction and often carries a real contribution deadline. Log the deadline. Route to `/regulatory-legal:policy-diff` only as a pre-positioning analysis, not as a gap-closure diff. |
| Enforcement action | Sector match → material; related-practice match → review-worthy; neither → FYI or skip |
| Guidance / orientação técnica | Review-worthy |
| Speech / blog / statement | FYI or skip per threshold |
| Settlement / TAC (termo de ajustamento de conduta) | Depends — novel theory or big number → review-worthy; routine → skip |

**Pre-normativo handling — specific.** Brazilian agencies don't run a uniform pre-rule process the way the US APA structures ANPR/RFI before NPRM — each agency (ANPD, BCB, ANATEL, etc.) has its own procedure name and cadence for a pre-consultation step, when it runs one at all `[model knowledge — verify against the specific agency's regimento]`. Treat any such item as a separate branch:

- **Do not** classify a pre-normativo item as "always material" — the compliance impact is zero until a norma issues.
- **Do** classify as review-worthy if any of the issue areas in the notice touch the watchlist's always-material categories (e.g., a BCB tomada de subsídios on open finance in a fintech watchlist).
- **Do** log the deadline to `~/.claude/plugins/config/claude-for-legal/regulatory-legal/comment-tracker.yaml` with `item_type: pre-normativo` so the downstream tracker can distinguish these from compliance gaps.
- **Do** include in the digest entry a line that says explicitly: "Pre-normativo. Deadline [date]. Route to `/regulatory-legal:policy-diff` only as a pre-positioning analysis (no compliance gap yet)." This primes the policy-diff skill to use its compressed pre-positioning branch rather than a full gap-closure diff.
- **Route to the comment-tracker, not the gap-tracker.** Consultation-decision items are not compliance gaps; they belong in the comment tracker, and `gap-surfacer` uses the `comment-decision` `gap_type` (or declines to ingest, if the team routes these separately).

**Consulta pública deadline handling:**

For every proposta em consulta pública classified at any tier above "skip":
- Extract the consultation deadline (structured data if the agency's own portal provides it; otherwise read the DOU publication or the consultation notice itself — there's no cross-agency structured API like the Federal Register's)
...
- Include the deadline in the output entry
...
**Feeds checked:** [list active tiers — e.g., "DOU, ANPD RSS, paid feed provider"]
**Items found:** [N] total
...
[Propostas em consulta pública: include "💬 Deadline: [date] — decision pending" if comment tracking enabled]
...
**Comment tracker:** [N] propostas em consulta pública with open decisions — run /regulatory-legal:comments to review

---

**Verify citations before relying on them.** Regulatory citations here were AI-generated and have not been checked against a primary source. Before acting on any rule, guidance, or enforcement action above, confirm it against JusBrasil, Escavador, your firm's research platform, or the issuing authority's own gov.br page — check accuracy, effective date, and current status. AI-generated regulatory citations are sometimes fabricated, misquoted, or stale. Source tags on each item (e.g., `[DOU]`, `[web search — verify]`) show where the citation came from; `verify` tags carry higher fabrication risk and should be checked first.
```
...
- **Feed configuration empty:** run the DOU check only and append: "This output used only the free DOU (Diário Oficial da União) check because your configuration doesn't list direct RSS or paid feeds. Add feeds with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.claude/plugins/config/claude-for-legal/regulatory-legal/CLAUDE.md`."
...
- **To comment-tracker:** Any proposta em consulta pública classified above "skip" → deadline logged automatically if tracking is enabled.
...
- Require a paid regulatory feed or JusBrasil/Escavador/PJe. Free feeds are the baseline; paid feeds add depth.

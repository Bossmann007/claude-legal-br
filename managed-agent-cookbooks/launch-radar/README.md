...
- **Trigger keywords.** The keyword list in the `launch-watcher` system prompt is opinionated (LGPD, ANPD, ANVISA, AI vendor names, etc., with optional US-footprint terms for cross-border products: COPPA, HIPAA, FedRAMP, TCPA, etc.). Delete categories that don't apply to your product, add domain-specific terms (BACEN/PIX, CADE, CDC, biometrics, etc.), and retune severity thresholds against your calibration table. Re-deploy after changes.

...
Regulatory feed content (DOU publications, per-agency gov.br portal pages, paid feed alerts) is **untrusted input.** Three-tier isolation:
...
- **Point `feed-reader` at your sources.** Unlike the US Federal Register, Brazil has no single structured public API for regulatory publication. The default target is the DOU (Diário Oficial da União — in.gov.br), split across seções I/II/III, which is RSS/scraping-based, plus the relevant per-agency gov.br portals (ANPD, BCB/CMN, ANVISA, ANATEL, ANEEL, ANS, CADE, depending on your practice area). This is a meaningfully different integration shape than the US version's "one clean API call" — expect to build or wire a page-watch/scrape job per source rather than a single feed endpoint, and flag this to whoever deploys this cookbook. If your firm subscribes to a paid regulatory feed (e.g., JOTA PRO, ConJur assinatura — `[user provided]`, since the paid feed a firm uses is client-specific) or uses direct agency alerts, add the endpoints to the feed-reader's web_fetch allowlist and adjust the orchestrator's scan plan.

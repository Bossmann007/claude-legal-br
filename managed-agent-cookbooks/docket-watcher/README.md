...
Monitors court dockets for matters in the active litigation portfolio. JusBrasil / Escavador cover case-law and docket search/aggregation across tribunals; PJe covers direct access to the electronic court records of each tribunal that runs a PJe instance (Justiça Estadual and Justiça Federal both use PJe, but each tribunal — TJ, TRF, TST, etc. — runs its own separate PJe deployment; there is no single unified PJe API across all courts). For each active matter the agent pulls new filings since the last check, maps filing types to candidate deadlines, cross-references against the matter's history and open deliverables, and produces a docket status report plus a structured deadline feed.
...
export JUSBRASIL_MCP_URL=...
export ESCAVADOR_MCP_URL=...
export PJE_MCP_URL=...
...
|---|---|---|---|
| **`docket-reader`** | **Yes** | `Read`, `Grep` only | jusbrasil, escavador, pje (read-only) |
...
- **Set the MCP URLs.** `JUSBRASIL_MCP_URL` and `ESCAVADOR_MCP_URL` must point at your deployment's endpoints for those aggregator/search platforms, and `PJE_MCP_URL` at whichever tribunal-specific PJe instance(s) your portfolio needs — remember each tribunal (TJ, TRF, TST, etc.) runs its own PJe deployment, so a multi-tribunal portfolio may need more than one PJe endpoint or a broker in front of several. `[verify]` exact API domains/auth against actual JusBrasil, Escavador, and PJe developer docs before production use. `GDRIVE_MCP_URL` (or a substitute) points at wherever your jurisdiction-rule tables live.
...
- **Configure jurisdiction rules.** Ship the deadline-mapper a local-rule table for every tribunal in your portfolio. The CPC (Código de Processo Civil) deadline rules you can encode once as a baseline; each tribunal's own regimento interno, provimentos, and individual juízo practice are where the landmines live — Justiça Estadual (TJ) and Justiça Federal (TRF) both run PJe, but neither runs the *same* PJe, and neither follows identical local procedural rules. An unknown court/tribunal should produce `confidence: low` + `needs_verification: true`, never a silent default.

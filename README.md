...
Backlog fechado. Todos os 12 plugins de área de prática do marketplace (mais `external_plugins/cocounsel-legal`, mantido como está — plugin de terceiro da Thomson Reuters, branded em Westlaw por natureza, fora de escopo de adaptação) foram auditados por substância US-centric e corrigidos. Os `managed-agent-cookbooks/` com integrações de pesquisa jurídica (`docket-watcher`, `reg-monitor`) também foram adaptados.
...
**Gap conhecido, aberto:** `employment-legal/agents/leave-tracker.md` ainda descreve o regime de licenças 100% em doutrina US (FMLA/USERRA/ADA, `leave_type: FMLA/CFRA/PFL`) sem nenhum equivalente CLT (auxílio-doença/INSS, licença-maternidade/paternidade, Lei 11.770/2008, estabilidade acidentária) — não pego pelo grep por keyword porque falha por omissão, não por termo presente. Reescrita despachada.

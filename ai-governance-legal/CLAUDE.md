...
*Escrito pelo cold-start interview. Até lá, isto é um template — se aparecer
`[PLACEHOLDER]`, execute `/ai-governance-legal:cold-start-interview`.*

---

## Company profile

[Empresa] é [descrição — o que a empresa faz e quem são seus clientes]. *(De company-profile.md — edite lá para alterar em todos os plugins)*

**Papel regulatório (BR):** *Não definido em nível de empresa.* No marco legal
brasileiro de IA (PL 2338/2023, ainda em tramitação no Senado, e a
regulamentação setorial existente — Resoluções CD/ANPD, Res. CMN/BCB
4.658 e correlatas para instituições financeiras, RDC Anvisa para SaMD com IA),
o papel do agente (fornecedor/desenvolvedor, operador/implementador,
distribuidor) é avaliado **por sistema de IA** — ver `## AI system inventory`
abaixo. A mesma empresa pode ser fornecedora de um sistema e operadora de
outro; um rótulo único em nível de empresa produz respostas erradas.

**Resumo de atividade em IA:** [PLACEHOLDER — parágrafo descrevendo como IA
permeia a empresa: se desenvolve modelos próprios, usa IA de terceiros,
treina modelos, ou uma combinação. Orientação apenas — a classificação
por sistema fica em `ai-systems.yaml`.]

**Perímetro regulatório:** [PLACEHOLDER — listar só o que realmente se aplica:
LGPD (Lei 13.709/2018, especialmente Art. 20 — decisões automatizadas),
PL 2338/2023 (se/quando sancionado), Resoluções ANPD, regulação setorial
(BCB/CMN para fintech, Anvisa/RDC para SaMD, CVM para mercado de capitais),
normas técnicas (ABNT NBR ISO/IEC 42001), exigências contratuais de clientes.
Se nada se aplica ainda, diga isso.] *(De company-profile.md — edite lá para alterar em todos os plugins)*
...
O marco legal brasileiro de IA é multicamadas: **LGPD já em vigor** (Art. 20
— direito a revisão de decisão automatizada; Art. 6º — princípios aplicáveis
a tratamento de dados que alimenta modelos), **PL 2338/2023** ainda em
tramitação no Senado (texto aprovado no Senado em 2024, pendente na Câmara —
`[verificar status atual]`), e **regulação setorial** já vigente que já
impõe obrigações a sistemas de IA específicos (Res. CMN/BCB para crédito e
scoring, RDC Anvisa para software como dispositivo médico, normas CVM para
robo-advisors). **Papel e nível de risco são avaliados por sistema de IA,
não por empresa.** A mesma organização pode ser fornecedora do Sistema A e
operadora do Sistema B — cada combinação aciona um conjunto diferente de
obrigações. Este inventário guarda um registro por sistema.

Cada registro carrega:
- `role` — fornecedor/desenvolvedor (quem projeta ou treina o sistema) / operador/implementador (quem usa o sistema em suas atividades) / distribuidor
- `role_basis` — explicação de uma frase de por que esse papel se aplica, tag `[verificar contra texto vigente do PL 2338/2023 e regulação setorial]`
- `tier` — risco excessivo (proibido) / alto risco / risco não classificado como alto (regime geral de transparência) — categorias do PL 2338/2023, Art. 13-17 e Anexo (numeração sujeita a mudança até sanção — `[verificar]`)
- `tier_basis` — o dispositivo ou hipótese do PL que enquadrou o sistema (ex.: sistemas usados em processos seletivos de emprego, análise de crédito, saúde, segurança pública — hipóteses de alto risco do Art. 14), tag `[verificar contra texto vigente]`
- `lgpd_overlay` — se o sistema envolve decisão automatizada que afeta a pessoa (Art. 20 LGPD) — direito a revisão humana e explicação, independente do status do PL 2338
- `br_nexus` — se o sistema afeta titulares de dados no Brasil ou está sujeito a regulação setorial brasileira
- `obligations_note` — nota curta sobre quais obrigações avaliar; não é uma tabela derivada automaticamente
- `next_review` — data e gatilho para reclassificação (ex.: sanção do PL 2338, nova Resolução ANPD)

**O inventário NÃO deriva obrigações automaticamente.** Quando o usuário
perguntar "quais são minhas obrigações para o Sistema X?", a resposta é
produzida em conversa, marcada `[verify]`, e roteada para
`/ai-governance-legal:aia-generation` para a avaliação de impacto formal se
necessário. Isso é deliberado — o PL 2338/2023 ainda está em tramitação, a
regulação setorial muda por resolução administrativa (ANPD, BCB, Anvisa), e
uma tabela fixa papel × nível → obrigações é exatamente o tipo de artefato
confiante-e-errado que acaba num memorando de conselho. O inventário é um
registro para o advogado; o advogado é dono da análise de obrigações.

Gerencie o inventário com `/ai-governance-legal:ai-inventory` —
...
- **Brasil:** não existe "attorney work product". A proteção análoga é o sigilo profissional do advogado (Art. 7º, XIX, Estatuto da OAB — Lei 8.906/1994; Art. 34-36, Código de Ética OAB), válida só quando o documento foi produzido ou supervisionado por advogado(a) inscrito(a) na OAB. A ANPD tem poder de fiscalização amplo (Art. 55-J LGPD) e um inventário de sistemas de IA, uma AIA (avaliação de impacto) ou uma nota de triagem NÃO são automaticamente protegidos de requisição da autoridade — a proteção depende de terem sido produzidos por advocacia, no contexto de assessoria jurídica, não de compliance operacional.
...
> **⚠️ Reviewer note**
> - **Sources:** [Research connector: JusBrasil/Escavador/PJe ✓ verified | not connected — cites from training knowledge, verify before relying]
...
- `[statute / regulator site]` — ONLY if you fetched the text from the regulator's website (gov.br/anpd, in.gov.br/DOU, or an official source) in this session.
...
- **`[settled — last confirmed YYYY-MM-DD]`** — stable statutory and regulatory references that have been checked against a primary source on the stated date. The date matters: "stable" references change. ANPD's regulamentações and the marco de IA (PL 2338/2023) have shifted repeatedly — a reference `[settled]` last year may not be today. The date tells the reader when the confidence was earned and whether it's earned it lately. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead — an unconfirmed "settled" is the confident overclaim we built the whole attribution system to prevent.
...
1. **Primary: the official register or regulator.** DOU (Diário Oficial da União — RSS/scraping-based across seções I/II/III, no unified structured API), gov.br agency portals (ANPD, BCB/CMN, Anvisa, CVM), Planalto (legislação federal), EUR-Lex, legislation.gov.uk, Federal Register (US), the regulator's own website (ICO, CNIL, EDPB, etc., for comparative reference). Tag `[primary source]`.

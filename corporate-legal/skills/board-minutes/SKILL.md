...
name: board-minutes
description: >
  Redige atas de reunião de sócios (Ltda.), assembleia de acionistas ou reunião
  de conselho de administração (S.A.) no formato da casa. Detecta
  automaticamente reuniões próximas no calendário, pede a pauta e materiais de
  apoio, e produz um rascunho completo no formato aprendido das atas-modelo.
  Também trata deliberação por escrito no lugar de reunião. Gatilho: "ata de
  reunião", "rascunho de ata", "próxima reunião de sócios", "assembleia",
  "reunião de conselho", "deliberação por escrito", ou detecção no calendário
  de evento societário próximo.
---
...
- Verifique o Contrato Social (Ltda.) ou Estatuto Social (S.A.) pelo requisito de quórum. Se omisso, aplique o padrão legal: Ltda. — quórum de instalação e deliberação conforme Art. 1.076, Código Civil (varia por matéria: maioria do capital social para a maioria das deliberações; unanimidade para certas alterações, salvo previsão diversa no contrato). S.A. — quórum de instalação em primeira convocação de 2/3 do capital votante e, em segunda convocação, qualquer número (Art. 125, Lei 6.404/1976), salvo quórum qualificado para matérias específicas (Art. 136). Registre o que confirmou (fonte e artigo) nas notas de redação.
- Confirme que o quórum estava presente. Se não: pare e sinalize antes de redigir. Não produza ata que implique reunião válida. Leve a questão ao advogado responsável — o caminho de remediação (ratificação, nova reunião, deliberação por escrito) depende do tipo societário e da natureza do ato.
...
**Header block:**
```
ATA DE [TIPO DE REUNIÃO — Reunião de Sócios / Assembleia Geral Ordinária ou
...
[Se houver deliberação:]
Submetida a matéria à votação, foi deliberado [por unanimidade / por maioria, com N votos favoráveis, N contrários, N abstenções]:

DELIBERAM os [sócios / acionistas / conselheiros], [texto da deliberação em linguagem da casa de `~/.claude/plugins/config/claude-for-legal/corporate-legal/CLAUDE.md`].
```

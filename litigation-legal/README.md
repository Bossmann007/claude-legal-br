...
**Conectores de consulta pública (read-only) disponíveis:** DataJud (metadados/movimentos de processos públicos) e DJEN (intimações do Diário de Justiça Eletrônico Nacional) — ver `connectors/` e `CONNECTORS.md`. **Não há** conector de escrita/protocolo nem acesso a autos de PJe/e-SAJ/Projudi ou a processos em segredo de justiça — esses dados continuam sendo inseridos manualmente (colados ou upload). Dar ciência, protocolar e consultar processo sigiloso se fazem pelos meios oficiais com credencial habilitada.
...
The legal research connectors in this plugin aren't just data sources — they're the difference between a verified citation and a citation you have to check. A citation retrieved through **JusBrasil** (jurisprudência, doutrina, legislação — searchable and traceable to source) or **Escavador** (processos e movimentações — dados de partes, andamentos, analytics de juízes e advogados adversos, every record cited to source) is tagged with its source and can be traced back. Docket status itself comes from whichever e-filing system the matter's tribunal actually runs — **PJe** (Justiça Federal and most state/labor courts), **e-SAJ**, **e-Proc**, or **Projudi** — tagged by system so you know which court's docket the movement came from. A citation from the model's knowledge or from web search is tagged `[verify]` or `[verify-pinpoint]` and should be checked against a primary source before anyone relies on it. The plugin tiers its citations so your verification time goes where it matters.

...
> **Note on terminology.** This skill keeps the US name "privilege log" because that's the plugin's file/command name, but Brazilian civil procedure has no broad discovery obligation and no standard cataloged log of withheld documents. The functional equivalent arises only when a court orders **exibição de documento ou coisa** (CPC arts. 396-404) and the responding party invokes a ground to refuse — most commonly **sigilo profissional do advogado** under CPC art. 404 combined with the Estatuto da OAB (Lei 8.906/1994) art. 7º, XIX. This skill reviews that **justificativa de recusa de exibição** entry by entry, applying the same underlying discipline (defend which documents are properly withheld and why) mapped onto the real BR mechanism.

1. Load `~/.claude/plugins/config/claude-for-legal/litigation-legal/CLAUDE.md` → review protocol, format of the justificativa de recusa.
2. Follow the workflow and reference below.
3. For each entry: obvious sigilo / obvious not sigilo / needs attorney review. Flag reasons.
4. Output: reviewed justificativa with flags. Attorney reviews all flags before responding to the order for exibição.

---

# Review of Justificativa de Recusa de Exibição (sigilo profissional)
...
When this skill cites a rule, local variant, or authority for a sigilo profissional call (CPC art. 404, Estatuto da OAB art. 7º, XIX, provimento local, precedente sobre alcance da renúncia/waiver, precedente sobre finalidade predominante), two rules apply.
...
`~/.claude/plugins/config/claude-for-legal/litigation-legal/CLAUDE.md` → format da justificativa de recusa, review protocol.

**Conflicts gate — unbypassable.** Before reviewing a justificativa de recusa de exibição, check `~/.claude/plugins/config/claude-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
**Before reviewing entries, research the requirements for a valid recusa de exibição sob sigilo profissional (CPC art. 404; Estatuto da OAB art. 7º, XIX), any provimento local, and relevant precedente. Identify the required elements of the justificativa, the level of description needed without waiving the very secrecy invoked, and any accommodation for describing documents by category. Cite primary sources.**

**No silent supplement.** If a research query to the configured legal research tool (JusBrasil, Escavador, PJe, or firm platform) returns few or no results for the applicable rule, waiver/renúncia doctrine, or local variant, report what was found and stop. Do NOT fill the gap from web search or model knowledge without asking. Say: "The search returned [N] results from [tool]. Coverage appears thin for [rule / doctrine]. Options: (1) broaden the search query, (2) try a different research tool, (3) search the web — results will be tagged `[web search — verify]` and should be checked against a primary source before relying, or (4) leave the `[UNCERTAIN]` marker and stop here. Which would you like?" A lawyer decides whether to accept lower-confidence sources; the skill does not decide for them.
...
**Before the justificativa de recusa is protocolada / apresentada ao juizo (the consequential act -- this includes filing the justificativa AND any designation of documents produced under a sigilo/confidencialidade protective order):** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:

> Filing a justificativa de recusa and designating documents under sigilo both have legal consequences -- over-invoking sigilo risks a determinacao de exibicao forcada and possible sancoes; under-invoking waives protection; a misdesignated production may be unrecallable. Have you reviewed this with an attorney? If yes, proceed. If no, here's a brief to bring to them:
>
> [Generate a 1-page summary: the matter, entry counts, the flags and close calls, pattern observations (over-invocation, vague descriptions), waiver posture, what could go wrong on filing or designation, what to ask the attorney.]
>
...
Do not treat the justificativa as filing-ready without an explicit yes. First-pass review, sorting, and flagging do not require the gate -- filing and designation do.

```markdown
[WORK-PRODUCT HEADER — per plugin config ## Outputs — differs by role; see `## Who's using this`]

## Review of Justificativa de Recusa de Exibição: [Matter] — [date]

**Applicable rule:** [CPC art. 404 (motivos legítimos de recusa de exibição) e Estatuto da OAB (Lei 8.906/1994) art. 7º, XIX (sigilo das comunicações e instrumentos de trabalho do advogado) — pinpoint cites] `[UNCERTAIN — verify currency]`
**Entries reviewed:** [N]

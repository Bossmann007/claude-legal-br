...
> This plugin can work with: DMS (iManage), document storage (Google Drive, SharePoint, Box), Gmail, scheduled-tasks, CLM (Ironclad), gestão documental/produção de prova documental (ferramentas de organização de documentos para exibição judicial conforme CPC art. 396-404), legal research (JusBrasil, Escavador, PJe). Let me check which connectors you have configured — features that need them will work, and features that don't will fall back gracefully instead of failing silently.

...
**The header's protection maps to a different doctrine here.** "Attorney work product" as a broad discovery-shielding privilege-log concept is a US doctrine built on FRCP 26(b)(3) — a rule that exists because US civil procedure has sweeping pretrial discovery to shield material *from*. Brazilian civil procedure has no equivalent discovery regime: document production is narrow and request-specific, governed by CPC art. 396-404 ("exibição de documento ou coisa"), so there is no privilege-log apparatus to defend against in the US sense. The functional protection here is **sigilo profissional do advogado** (the lawyer's professional secrecy) and the **prerrogativa da defesa**, grounded in the Estatuto da OAB (Lei 8.906/1994), art. 7º — notably:

- **Inciso II:** inviolability of the lawyer's office/workplace, files, correspondence, and electronic/telephone communications, provided they relate to the exercise of the profession.
- **Inciso XIX:** the right to refuse to testify about facts that constitute professional secret, and the corresponding duty of confidentiality owed to the client.

This is a standing statutory inviolability of the lawyer's work instruments and client communications — enforceable against search/seizure and against compelled testimony, independent of whether litigation is pending — not a discovery-response shield. It protects less in some ways (there is no discovery to fight over) and more in others (the OAB has historically resisted search-and-seizure of lawyers' offices even under judicial warrant, citing art. 7º).

**When the practice profile's jurisdiction footprint includes non-Brazilian jurisdictions,** adjust the header:
- Keep `PRIVILEGED & CONFIDENTIAL` / `SIGILO PROFISSIONAL — PRERROGATIVA DA OAB` (confidentiality markings are meaningful everywhere).
- Add a jurisdiction note: `[Nota: a proteção invocada aqui é o sigilo profissional do art. 7º do Estatuto da OAB (Lei 8.906/1994), não o "work product" norte-americano. Em [jurisdição], o regime de proteção é diferente — confirme o regime de sigilo/confidencialidade aplicável antes de usar esta marcação para impedir divulgação.]`
- For matters touching the US or EU: consider `CONFIDENTIAL — INTERNAL LEGAL ANALYSIS — NOT A SUBSTITUTE FOR LOCAL COUNSEL ADVICE`, which is honest and doesn't assert a protection regime that doesn't apply there.

A false assurance of protection is worse than no marking. The lawyer who invokes Brazilian sigilo profissional to shield a document from a foreign regulator (or vice versa) is the lawyer who loses the argument.
...
> "You mentioned a prazo prescricional de 5 anos para essa verba trabalhista contando da rescisão — meu entendimento é que, nos termos do art. 7º, XXIX da CF/88, o prazo é de 5 anos na constância do contrato, limitado a 2 anos após a extinção do vínculo. Pode confirmar o que você quis dizer? `[premise flagged — verify]`"
...
- `[statute / regulator site]` — ONLY if you fetched the text from the regulator's website (e.g., planalto.gov.br, gov.br/anpd) or an official source in this session.
...
- **`[settled — last confirmed YYYY-MM-DD]`** — stable statutory and regulatory references that have been checked against a primary source on the stated date. The date matters: "stable" references change. ANPD's evolving regulamentação under the LGPD (Lei 13.709/2018) has changed enforcement guidance and normative resolutions repeatedly since 2020; interpretations of the Marco Civil da Internet (Lei 12.965/2014) continue to shift with case law; the PL 2338/2023 (AI Bill) status has moved between committee stages more than once. The date tells the reader when the confidence was earned and whether it's earned it lately. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead — an unconfirmed "settled" is the confident overclaim we built the whole attribution system to prevent.
...
**Frequent fora:** [PLACEHOLDER — e.g., TJSP (1ª Vara Empresarial da Capital), TJRJ, Justiça Federal (Seção Judiciária de São Paulo / Rio de Janeiro), Câmara de Arbitragem CAM-CCBC / CAMARB]
